...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addToCart = (product: Product, pack: "trial" | "value", quantity = 1) => {
    setItems((prev) => {
      const existing = prev.find((i) => i.product.id === product.id && i.pack === pack);
      if (existing) {
        return prev.map((i) =>
  ...
  const removeFromCart = (productId: string, pack: "trial" | "value") => {
    setItems((prev) => prev.filter((i) => !(i.product.id === productId && i.pack === pack)));
  };
  ...
  const updateQuantity = (productId: string, pack: "trial" | "value", quantity: number) => {
    ...
    setItems((prev) =>
      prev.map((i) =>
        i.product.id === productId && i.pack === pack ? { ...i, quantity } : i
      )
    );
  };
